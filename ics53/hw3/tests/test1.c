...
/*
  void* k = ics_malloc(10000); 
  ics_payload_print(k); 
  ics_freelist_print(); 
  

  void* d = ics_malloc(100);
  printf("payload_print for d"); 
  ics_payload_print(d); 
  printf("first time"); 
  ics_freelist_print(); 

  ics_realloc(d, 0); 
  printf("payload_print for p");
  //ics_payload_print(p); 
  printf("second time"); 
  ics_freelist_print(); 
...
*/
  ...
  int *value1 = ics_malloc(sizeof(int));
  ics_freelist_print();
  int *value2 = ics_malloc(sizeof(long)); 
  ics_freelist_print();
  ics_free(value1);
  ics_payload_print((void*)value1);
  ics_free(value2);
  ...
  printf("=== Test7: xjb tests ===\n");
  printf("test1\n\n"); 
  void* hello = ics_malloc(26); 
  ics_payload_print(hello); 
  ics_freelist_print(); 
  printf("test2\n\n"); 
  printf("%p\n", ics_realloc(hello, 1000000)); 
  printf("test3\n\n"); 
  printf("%p\n", ics_realloc(hello, 10)); 
  //printf("%p", ics_realloc(hello, 1000000)); 
  printf("test4\n\n"); 
  void* newPtr = ics_realloc(hello, 35); 
  ics_payload_print(hello); 
  ics_freelist_print(); 
  printf("test5\n\n"); 
  newPtr = ics_realloc(newPtr, 7); 
  ics_payload_print(newPtr); 
  ics_freelist_print(); 
